 ...
 * \brief		Page to make breakdown of sales lines of customers invoices 
 */
...
// A REVOIR elarifr si encore necessaire de rajouter , p.fk_product_type as type_p 
//$sql .= " p.rowid as product_id, p.ref as product_ref, p.label as product_label, l.product_type as type, p.accountancy_code_sell as code_sell, p.fk_product_type as type_p";
...
// A REVOIR elarifr si encore necessaire de modifier order-by : pbr si modif facture les lignes ne sont plus dans l'ordre rowid
...
if ($result) {
	...
	print '<td align="right">' . $langs->trans("Compte") . '</td>';	//AccountAccounting  change to long label for a row
	print '<td align="left">' . $langs->trans("IntoAccount") . '</td>';
	...
	while ( $i < min($num_lignes, $limit) ) {
		...
		print '<td>' . dol_trunc($objp->description, ACCOUNTINGEX_LENGTH_DESCRIPTION) . '</td>';
		...
		//elarifr display rowid as in customer/lignes.php
